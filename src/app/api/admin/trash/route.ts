...
import { getStoreTrash, restoreStoreItem, purgeStoreItem } from '@/lib/store';
...
export async function GET() {
  try {
    ...
    try {
      ...
      console.warn('Prisma trash fetch fallback to store:', dbErr);
    }

    if (trashItems.length === 0) {
      trashItems = getStoreTrash();
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Always restore in store
    restoreStoreItem(entityType, id, session);
    ...
    try {
      ...
      console.warn('Prisma restore fallback to store:', dbErr);
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // Always purge in store
    purgeStoreItem(entityType, id, session);
    ...
    try {
      ...
      console.warn('Prisma permanent delete fallback to store:', dbErr);
    }
