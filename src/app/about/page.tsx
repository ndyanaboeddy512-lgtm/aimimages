    ...
    <div className="pt-32 pb-24 space-y-24">
      ...
      <div className="max-w-7xl mx-auto px-6 space-y-12">
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <h1 className="font-serif text-4xl sm:text-6xl font-bold text-cream-50 leading-tight">
        ...
        {/* Hero Image Showcase */}
        <div className="relative aspect-[21/9] rounded-3xl overflow-hidden bg-obsidian-900 border border-white/10 shadow-2xl">
          <Image
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 p-8 sm:p-12 rounded-2xl bg-obsidian-900 border border-white/5">
          {STATS.map((stat, idx) => (
            <div key={idx} className="text-center space-y-1">
              <span className="font-serif text-3xl sm:text-5xl font-bold gold-gradient-text block">
                {stat.value}
              </span>
      ...
      <div className="max-w-7xl mx-auto px-6 space-y-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {values.map((v, i) => (
            <div
              key={i}
              className="p-8 rounded-2xl bg-obsidian-850 border border-white/10 space-y-4 hover:border-gold-500/30 transition-colors"
            >
              ...
              <h3 className="font-serif text-xl font-bold text-cream-50">{v.title}</h3>
              ...
              >
                <div>
                  ...
                  <div className="p-6 space-y-2">
                    <h3 className="font-serif text-lg font-bold text-cream-50 group-hover:text-gold-400 transition-colors">
                      {member.name}
                    </h3>
          ...
          {BTS_IMAGES.map((item, idx) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent p-5 flex flex-col justify-end">
                <span className="font-serif text-sm font-bold text-cream-50">{item.title}</span>
                <span className="text-[11px] text-gold-400">{item.subtitle}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Bottom CTA */}
      <div className="max-w-7xl mx-auto px-6">
        <div className="p-12 rounded-3xl bg-gradient-to-r from-obsidian-900 via-obsidian-850 to-obsidian-900 border border-gold-500/30 text-center space-y-6">
          ...
          <div className="pt-2">
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 px-8 py-4 rounded-full bg-gold-500 hover:bg-gold-400 text-obsidian-950 font-bold text-xs uppercase tracking-widest transition-colors shadow-lg"
            >
